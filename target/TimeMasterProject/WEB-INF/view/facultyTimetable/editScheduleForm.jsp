...
<body>
    ...
    <div class="content">
		...
		<form class="schedule-form-test" action="/TimeMasterProject/faculty/timetable/schedule/update" method="POST">
		    ...
		    <select class="select-course" id="course" name="course" onchange="loadSections()">
		        <option value="" selected disabled>Select Course</option>
		        <c:forEach var="course" items="${availableCourses}">
		            <option value="${course.courseCode}" ${course.courseCode == selectedCourse ? 'selected' : ''}>
		    ...
		    <select class="select-lecturer" id="lecturer" name="lecturer">
		        ...
		        <c:forEach var="lecturer" items="${availableLecturers}">
		            <option value="${lecturer.lecturerID}" ${lecturer.lecturerID == selectedLecturer ? 'selected' : ''}>
		                ${lecturer.name}
		            </option>
		        </c:forEach>
		    </select>
