...
<body>
	...
	<div class="pagination">
	    <c:if test="${currentPage > 1}">
	        <a href="?timetableID=${param.timetableID}&page=${currentPage - 1}&pageSize=${pageSize}">Previous</a>
	    </c:if>
	    <c:forEach begin="1" end="${totalPages}" var="i">
	        <c:choose>
	            ...
	            <c:otherwise>
	                <a href="?timetableID=${param.timetableID}&page=${i}&pageSize=${pageSize}">${i}</a>
	            </c:otherwise>
	        </c:choose>
	    </c:forEach>
	    <c:if test="${currentPage < totalPages}">
	        <a href="?timetableID=${param.timetableID}&page=${currentPage + 1}&pageSize=${pageSize}">Next</a>
	    </c:if>
	</div>
